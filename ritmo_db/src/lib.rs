...
pub async fn create_sqlite_pool(database_url: &str) -> RitmoResult<SqlitePool> {
    let pool = SqlitePoolOptions::new()
        ...
        .await
        .map_err(|err| {
            RitmoErr::DatabaseConnection(format!("failed to connect to sqlite: {err}"))
        })?;
    ...
    sqlx::raw_sql(SEED_LOOKUPS_SQL)
        ...
        .await
        .map_err(|err| {
            RitmoErr::DatabaseMigration(format!("failed to run seed_lookups.sql: {err}"))
        })?;
    ...
    sqlx::raw_sql(SEED_PAGE_FIELDS_SQL)
        ...
        .await
        .map_err(|err| {
            RitmoErr::DatabaseMigration(format!("failed to run seed_page_fields.sql: {err}"))
        })?;
